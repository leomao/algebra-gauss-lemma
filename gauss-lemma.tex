...
\begin{definition}
  Let $R$ be a GCD and $P = a_nx^n + \dots + a_1x + a_0 \in R[x]$.
  We define the {\bf content} of $P$ as $c(P) = \gcd(a_n, \dots, a_0)$.
\end{definition}

\begin{definition}
  Let $R$ be a GCD and $P = a_nx^n + \dots + a_1x + a_0 \in R[x]$.
  We say that $P$ is {\bf primitive} if $c(P) = 1$.
\end{definition}

\begin{theorem}[Gauss's Lemma]
  Let $R$ be a UFD and $A, B \in R[x]$ are primitive. Then $AB$
  is also primitive.
  \begin{proof}
    ...
    $AB$. Since $R$ is a UFD, there exists an irreducible $p \in R$
    ...
    a ring homo.), we have $0 = \tau(AB) = \tau(A)\tau(B)$. Since
    $(\quot{R}{I})[x]$ is an integral domain, one of $\tau(A), \tau(B)$ must
...
\begin{theorem}
  Let $R$ be a UFD and $F$ is the fraction field of $R$. If $P \in R[x]$
  is reducible in $F[x]$, then $P$ is also reducible in $R[x]$.
\end{theorem}

\begin{remark}
  Let $R$ be a UFD and $F$ is the fraction field of $R$. $P \in R[x]$
  is irreducible $\iff$ $P$ is irreducible in $F[x]$ and primitive in $R[x]$.
\end{remark}
